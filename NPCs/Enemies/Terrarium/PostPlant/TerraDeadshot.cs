    ...
    public class TerraDeadshot : ModNPC
    {
            ...
            if (Main.netMode != 1)
            {
                ...
                if (npc.localAI[2] >= 360 + Main.rand.Next(360) && npc.Distance(Main.player[npc.target].Center) < 400f && Math.Abs(npc.DirectionTo(Main.player[npc.target].Center).Y) < 0.5f && Collision.CanHitLine(npc.Center, 0, 0, Main.player[npc.target].Center, 0, 0))
                {
                    ...
                    for (int num86 = 0; num86 < 4; num86++)
                    {
                        ...
                        Projectile.NewProjectile(vector13.X, vector13.Y, vector15.X - 5, vector15.Y - 5, Terraria.ModLoader.ModContent.ProjectileType<TerrariumArrow>(), num85, 1f, Main.myPlayer, 0f, 0f);
                    }
                }
            }
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            if (npc.life <= 0)
            {
                ...
                npc.position.X = npc.position.X - npc.width / 2;
                npc.position.Y = npc.position.Y - npc.height / 2;
                int dust1 = Terraria.ModLoader.ModContent.DustType<Dusts.RangedDust>();
                int dust2 = Terraria.ModLoader.ModContent.DustType<Dusts.RangedDust>();
