...
namespace AAMod.NPCs.Bosses.Broodmother
{
    ...
    public class Broodmother : ModNPC
    {
		...
		public override void AI()
        {
            ...
            int Minions = NPC.CountNPCS(Terraria.ModLoader.ModContent.NPCType<BroodEgg>()) + NPC.CountNPCS(Terraria.ModLoader.ModContent.NPCType<Broodmini>());
            ...
            if (internalAI[1] == AISTATE_FIREBREATH)
            {
                ...
                if (Main.netMode != 1)
                {
                    ...
                    if (internalAI[2] > 30f)
                    {
                        BaseAI.ShootPeriodic(npc, player.position, player.width, player.height, Terraria.ModLoader.ModContent.ProjectileType<BroodBreath>(), ref internalAI[3], 5, damage, 12, true, new Vector2(0, 40f));
                    }
                    if (internalAI[2] > 90)
                    {
                        internalAI[0] = 0;
            ...
            else if (internalAI[1] == AISTATE_SPAWNEGGS)
            {
                if (Main.netMode != 1)
                {
                    ...
                    if (projectileTimer >= projectileInterval && projectileTimer % 20 == 0)
                    {
                        ...
                        {
                            int NPCID = NPC.NewNPC((int)firePos.X, (int)firePos.Y, Terraria.ModLoader.ModContent.NPCType<BroodEgg>(), npc.whoAmI, 0f, 0f, 0f, 0f, 255);
