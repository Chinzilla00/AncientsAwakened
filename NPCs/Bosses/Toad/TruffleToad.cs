    ...
    public class TruffleToad : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (Shrooms != null && Shrooms.Length > 0)
            {
                ...
                if (Main.netMode != 2 && Main.LocalPlayer.miscCounter % 2 == 0)
                {
                    for (int m = 0; m < Shrooms.Length; m++)
                    {
                        ...
                        {
                            int dustID = Dust.NewDust(npc2.position, npc2.width, npc2.height, Terraria.ModLoader.ModContent.DustType<Dusts.ShroomDust>());
            ...
            else if (internalAI[0] == AISTATE_STOMP)
            {
                ...
                else
                {
                    if (npc.velocity.Y == 0f)
                    {
                        ...
                        for (int a = 0; a < 4; a++)
                        {
                            NPC.NewNPC((int)(npc.position.X + Main.rand.Next(40)), (int)(npc.position.Y + npc.height), Terraria.ModLoader.ModContent.NPCType<GlowshroomGrow>());
                        }
                        internalAI[0] = AISTATE_JUMP;
                        internalAI[1] = 0;
                        internalAI[2] = 0;
            ...
            else if (internalAI[0] == AISTATE_TOADS)
            {
                ...
                if (internalAI[1] == 35)
                {
                    NPC.NewNPC((int)(npc.Center.X - 30f), (int)(npc.Center.Y - 16), Terraria.ModLoader.ModContent.NPCType<TinyToad>());
                    NPC.NewNPC((int)npc.Center.X, (int)(npc.Center.Y - 16), Terraria.ModLoader.ModContent.NPCType<TinyToad>());
                    NPC.NewNPC((int)(npc.Center.X + 30f), (int)(npc.Center.Y - 16), Terraria.ModLoader.ModContent.NPCType<TinyToad>());
                }
                if (internalAI[1] >= 100)
                {
