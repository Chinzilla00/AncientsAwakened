    ...
    public class SagittariusFree : Sagittarius
	{
        ...
        public void ShootLaser(NPC npc, Vector2 velocity)
        {
            ...
            if (shootAI[0] >= 10)
            {
                ...
                for (int i = 0; i < Main.rand.Next(1, 3); i++)
                {
                    ...
                    Projectile.NewProjectile(player.position.X, player.position.Y, baseSpeed * (float)Math.Sin(offsetAngle), baseSpeed * (float)Math.Cos(offsetAngle), Terraria.ModLoader.ModContent.ProjectileType<Zero.DeathLaser>(), damage, 2, Main.myPlayer);
                }
            }
        }
        ...
        public override void NPCLoot()
        {
            ...
            if (!Main.expertMode)
            {
                ...
                Item.NewItem(npc.Center, Terraria.ModLoader.ModContent.ItemType<Items.Materials.Doomite>(), Main.rand.Next(20, 30));
            }
            else
            {
                npc.DropBossBags();
