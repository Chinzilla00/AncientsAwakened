...
namespace AAMod.NPCs.Bosses.Greed
{
    ...
    public class GreedA : ModNPC
    {
        ...
        public override bool PreAI()
        {
            ...
            if (npc.HasBuff(Terraria.ModLoader.ModContent.BuffType<BuffA>()))
            {
                npc.damage = (int)(npc.defDamage * 1.15f);
            }

            if (npc.HasBuff(Terraria.ModLoader.ModContent.BuffType<BuffD>()))
            {
            ...
            switch ((int)internalAI[0])
            {
                case 0:
                    if (++internalAI[1] > 60)
                    {
                        ...
                        internalAI[1] = 0;
                        npc.netUpdate = true;
                        if (NPC.CountNPCS(Terraria.ModLoader.ModContent.NPCType<GreedTurret>()) < 2)
                        {
                            int A = Main.rand.Next(-600, 600);
                            int Minion = NPC.NewNPC((int)player.Center.X + A, (int)player.Center.Y + A, Terraria.ModLoader.ModContent.NPCType<GreedTurret>(), 0);
            ...
            if (Main.netMode != 1)
            {
                if (npc.ai[0] == 0)
                {
                    ...
                    for (int i = 0; i < 30; ++i)
                    {
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, Terraria.ModLoader.ModContent.NPCType<GreedABody>(), npc.whoAmI, 0, latestNPC);
        ...
        {
            if (npc.type != Terraria.ModLoader.ModContent.NPCType<GreedA>())
            {
                return false;
            }
            scale = 1.5f;
        ...
        public void MinionSummon()
        {
            ...
            {
                int a = NPC.NewNPC((int)npc.Center.X + Xint, (int)npc.Center.Y + Yint, Terraria.ModLoader.ModContent.NPCType<GreedMinion>(), 0, 24);
                Main.npc[a].Center = new Vector2(npc.Center.X + Xint, npc.Center.Y + Yint);
                Xint = Main.rand.Next(-400, 400);
                Yint = Main.rand.Next(-400, 400);
                a = NPC.NewNPC((int)npc.Center.X + Xint, (int)npc.Center.Y + Yint, Terraria.ModLoader.ModContent.NPCType<GreedMinion>(), 0, 25);
                Main.npc[a].Center = new Vector2(npc.Center.X + Xint, npc.Center.Y + Yint);
                Xint = Main.rand.Next(-400, 400);
                Yint = Main.rand.Next(-400, 400);
                a = NPC.NewNPC((int)npc.Center.X + Xint, (int)npc.Center.Y + Yint, Terraria.ModLoader.ModContent.NPCType<GreedMinion>(), 0, 26);
                Main.npc[a].Center = new Vector2(npc.Center.X + Xint, npc.Center.Y + Yint);
                Xint = Main.rand.Next(-400, 400);
                Yint = Main.rand.Next(-400, 400);
                a = NPC.NewNPC((int)npc.Center.X + Xint, (int)npc.Center.Y + Yint, Terraria.ModLoader.ModContent.NPCType<GreedMinion>(), 0, 27);
                Main.npc[a].Center = new Vector2(npc.Center.X + Xint, npc.Center.Y + Yint);
            }
            else if (MinionChoice == 12)
            {
                int a = NPC.NewNPC((int)npc.Center.X + Xint, (int)npc.Center.Y + Yint, Terraria.ModLoader.ModContent.NPCType<GreedMinion>(), 0, 28);
                Main.npc[a].Center = new Vector2(npc.Center.X + Xint, npc.Center.Y + Yint);
            }
        }
    }
    ...
    public class GreedABody : GreedA
    {
        ...
        public override void FindFrame(int frameHeight)
        {
            if (npc.type == Terraria.ModLoader.ModContent.NPCType<GreedABody>())
            {
