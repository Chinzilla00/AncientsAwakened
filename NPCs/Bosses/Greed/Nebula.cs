...
namespace AAMod.NPCs.Bosses.Greed
{
    public class Nebula : ModProjectile
	{
        ...
        {
            Projectile.NewProjectile(projectile.position.X, projectile.position.Y, 0, 0, mod.ProjectileType<NebulaBoost>(), 0, 0, Main.myPlayer, projectile.ai[0]);
            projectile.Kill();
        }
        ...
        public override void Kill(int timeleft)
        {
            ...
            {
                int dustID = Dust.NewDust(new Vector2(projectile.Center.X - 1, projectile.Center.Y - 1), 2, 2, mod.DustType<Dusts.AbyssDust>(), 0f, 0f, 100, c, 1.6f);
            ...
            for (int m = 0; m < pieCut; m++)
            {
                int dustID = Dust.NewDust(new Vector2(projectile.Center.X - 1, projectile.Center.Y - 1), 2, 2, mod.DustType<Dusts.AbyssDust>(), 0f, 0f, 100, c, 2f);
    ...
    public class NebulaBoost : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (projectile.ai[aislotHomingCooldown] > homingDelay)
            {
                ...
                int foundTarget = BaseAI.GetNPC(projectile.Center, mod.NPCType<GreedA>(), -1);
                if (foundTarget != -1)
                {
        ...
        {
            if (target.type == mod.NPCType<GreedA>())
            {
                if (projectile.ai[0] == 0)
                {
                    target.AddBuff(mod.BuffType<BuffA>(), 180);
                }
                else if (projectile.ai[0] == 1)
                {
                    target.AddBuff(mod.BuffType<BuffD>(), 180);
                }
                else
                {
                    target.life += 1000;
