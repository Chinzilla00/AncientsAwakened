...
namespace AAMod.NPCs.Bosses.Akuma.Awakened
{
    public class AkumaRock : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
			...
			{
				Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, Terraria.ModLoader.ModContent.DustType<Dusts.AkumaADust>(), 0f, 0f, 100, new Color(255, Main.DiscoG, 0), 1.5f);
			}
			for (int num337 = 0; num337 < 32; num337++)
			{
				int num338 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, Terraria.ModLoader.ModContent.DustType<Dusts.AkumaADust>(), 0f, 0f, 100, new Color(255, Main.DiscoG, 0), 2.5f);
				Main.dust[num338].noGravity = true;
				Main.dust[num338].velocity *= 3f;
				num338 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, Terraria.ModLoader.ModContent.DustType<Dusts.AkumaDust>(), 0f, 0f, 100, new Color(255, Main.DiscoG, 0), 1.5f);
			...
			for (int num341 = 0; num341 < 5; num341++)
			{
				...
				{
                    Terraria.ModLoader.ModContent.DustType<Dusts.AkumaADust>(),
                    Terraria.ModLoader.ModContent.DustType<Dusts.AkumaDust>()
                });
