...
namespace AAMod.NPCs.Bosses.Athena
{
    public class OlympianDragon : ModNPC
    {
        ...
        {
            if (!NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<Athena>()) || !NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<AthenaA>()))
            {
                npc.velocity *= .95f;
                if (npc.alpha != 0)
                {
                    ...
                    {
                        int num935 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, Terraria.ModLoader.ModContent.DustType<Dusts.AbyssDust>(), 0f, 0f, 100);
