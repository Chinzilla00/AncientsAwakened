    ...
    public class FuryAshe : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (player.dead || !player.active || Math.Abs(npc.position.X - Main.player[npc.target].position.X) > 6000f || Math.Abs(npc.position.Y - Main.player[npc.target].position.Y) > 6000f)
            {
                ...
                if (player.dead || !player.active || Math.Abs(npc.position.X - Main.player[npc.target].position.X) > 6000f || Math.Abs(npc.position.Y - Main.player[npc.target].position.Y) > 6000f)
                {
                    if (Main.netMode != 1)
                    {
                        int DeathAnim = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, Terraria.ModLoader.ModContent.NPCType<FuryAsheVanish>(), 0);
            ...
            if (internalAI[0] == AISTATE_HOVER || internalAI[0] == AISTATE_DRAGON) //Hovering/Summoning Dragon
            {
                if (Main.netMode != 1 && internalAI[0] == AISTATE_HOVER) //Only randomly select AI if not doing a dragon summon
                {
                    ...
                    {
                        internalAI[3] = 0;
                        if (NPC.CountNPCS(Terraria.ModLoader.ModContent.NPCType<Shenling>()) < 3)
                        {
            ...
            if (internalAI[0] == AISTATE_DRAGON) //Summoning a dragon
            {
                ...
                if (internalAI[3] > 240)
                {
                    NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, Terraria.ModLoader.ModContent.NPCType<Shenling>(), 0);
                    internalAI[0] = 0;
                    internalAI[1] = 0;
                    internalAI[2] = 0;
                    internalAI[3] = 0;
        ...
        public void FireMagic(NPC npc, Vector2 velocity)
        {
            ...
            if (internalAI[0] == 1)
            {
                if (Main.netMode != 1)
                {
                    for (int m = 0; m < 3; m++)
                    {
                        ...
                        int projectile = Projectile.NewProjectile((int)infernoPos.X, (int)infernoPos.Y, vel.X, vel.Y, Terraria.ModLoader.ModContent.ProjectileType<DiscordianInferno>(), npc.damage / 2, 0f, Main.myPlayer, 0f, 0f);
            ...
            else if(internalAI[0] == 2)
            {
                ...
                BaseAI.ShootPeriodic(npc, player.position, player.width, player.height, Terraria.ModLoader.ModContent.ProjectileType<ShenABreath>(), ref shootAI[0], 5, npc.damage / 2, 12);
            }
            else if(internalAI[0] == 3)
            {
                BaseAI.FireProjectile(player.Center, npc, Terraria.ModLoader.ModContent.ProjectileType<Akuma.Awakened.AkumaRock>(), npc.damage, 3, 5f, 0, 0, 0);
            }
            else if (internalAI[0] == 4)
            {
                BaseAI.FireProjectile(player.Center, npc, Terraria.ModLoader.ModContent.ProjectileType<Akuma.AkumaFireProj>(), npc.damage, 3, 5f, 0, 0, 0);
            }
        }
