...
namespace AAMod.NPCs.Bosses.Shen
{
    ...
    public class ShenA : Shen
    {
        ...
        public override void AI()
        {
            ...
            if (NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<FuryAshe>()) || NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<WrathHaruka>()))
            {
                if (npc.alpha > 50)
                {
                    npc.alpha = 50;
            ...
            else
            {
                if (npc.alpha > 0)
                {
                    ...
                    {
                        int dust = Terraria.ModLoader.ModContent.DustType<Dusts.DiscordLight>();
        ...
        public override void NPCLoot()
        {
            if (npc.type == Terraria.ModLoader.ModContent.NPCType<ShenA>())
            {
                if (Main.expertMode)
                {
                    ...
                    {
                        npc.DropLoot(Terraria.ModLoader.ModContent.ItemType<Items.BossSummons.ChaosRune>(), 1f / 7);
                    ...
                    if (!NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<ShenDefeat>()))
                    {
                        NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, Terraria.ModLoader.ModContent.NPCType<ShenDefeat>());
                    }
                    npc.DropBossBags();
                }
            }
