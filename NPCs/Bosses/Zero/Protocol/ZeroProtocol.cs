...
namespace AAMod.NPCs.Bosses.Zero.Protocol
{
    ...
    public class ZeroProtocol : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            {
                case 0:
                    Proj = Terraria.ModLoader.ModContent.ProjectileType<DeathLaser>();
                    break;
                default:
                    Proj = Terraria.ModLoader.ModContent.ProjectileType<GlitchBomb>();
                    break;
            }
        ...
        public void Attack(int Attack)
        {
            if (Attack == 0)
            {
                for (int i = 0; i < 4; i++)
                {
                    if (i == 1)
                    {
                        NPC.NewNPC((int)npc.Center.X + 30, (int)npc.Center.Y + 30, Terraria.ModLoader.ModContent.NPCType<NullZP>());
                    }
                    else if (i == 2)
                    {
                        NPC.NewNPC((int)npc.Center.X + 30, (int)npc.Center.Y - 30, Terraria.ModLoader.ModContent.NPCType<NullZP>());
