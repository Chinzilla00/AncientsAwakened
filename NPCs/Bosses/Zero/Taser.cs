                ...
                if (Collision.CanHit(npc.position, npc.width, npc.height, player.Center, player.width, player.height))
                {
                    ...
                    for (int num842 = 0; num842 < num838; num842++)
                    {
                        ...
                        for (int i = 0; i < 3; i++)
                        {
                            ...
                            Projectile.NewProjectile(npc.Center.X, npc.Center.Y, vector83.X , vector83.Y, Terraria.ModLoader.ModContent.ProjectileType<ZeroShock>(), npc.damage / 2, 0f, Main.myPlayer, vector82.ToRotation(), ai);
                        }
                    }
                }
            }
