...
namespace AAMod.Projectiles.Greed
{
    public class GoldFountain : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            int FountainCount = AAGlobalProjectile.CountProjectiles(mod.ProjectileType<GoldFountain>());
            if (FountainCount < 1) FountainCount = 1;
            if (Main.netMode != 1 && projectile.ai[0]++ >= 5 * FountainCount)
            {
                Projectile.NewProjectile(projectile.position.X, projectile.position.Y, Main.rand.Next(-3, 4), Main.rand.Next(7, 10), mod.ProjectileType<Gold>(), projectile.damage, 1);
                projectile.ai[0] = 0;
