...
{
    public class FireSpirit : ModProjectile
    {
        ...
        {
            bool flag64 = projectile.type == mod.ProjectileType("FireSpirit");
            Player player = Main.player[projectile.owner];
            AAPlayer modPlayer = player.GetModPlayer<AAPlayer>(mod);
            player.AddBuff(mod.BuffType("FireSpirit"), 3600);
            if (flag64)
            {
                if (player.dead)
                {
            ...
            if (projectile.ai[0] == 0f)
            {
                ...
                int num29 = mod.ProjectileType<FireSpiritShot>();
