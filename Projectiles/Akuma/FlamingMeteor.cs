...
namespace AAMod.Projectiles.Akuma
{
    public class FlamingMeteor : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			Dust dust20 = Main.dust[Dust.NewDust(projectile.position, projectile.width, projectile.height, mod.DustType<Dusts.AkumaDustLight>(), 0f, 0f, 0, default, 1f)];
			...
			dust20 = Main.dust[Dust.NewDust(projectile.position, projectile.width, projectile.height, mod.DustType<Dusts.AkumaADustLight>(), 0f, 0f, 0, default, 1f)];
			dust20.position = position;
			dust20.velocity = projectile.velocity.RotatedBy(-1.5707963705062866, default) * 0.33f + projectile.velocity / 4f;
			dust20.position += projectile.velocity.RotatedBy(-1.5707963705062866, default);
			dust20.fadeIn = 0.5f;
			dust20.noGravity = true;
            int num190 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, mod.DustType<Dusts.AkumaDustLight>(), 0f, 0f, 0, default, 1f);
        ...
        public override void Kill(int timeLeft)
        {
			...
			{
				Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, mod.DustType<Dusts.AkumaADustLight>(), 0f, 0f, 100, default, 1.5f);
			}
			for (int num337 = 0; num337 < 32; num337++)
			{
                int dust = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, mod.DustType<Dusts.AkumaDustLight>(), 0f, 0f, 100, default, 2.5f);
				Main.dust[dust].noGravity = true;
				Main.dust[dust].velocity *= 3f;
				dust = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, mod.DustType<Dusts.AkumaDustLight>(), 0f, 0f, 100, default, 1.5f);
			...
			for (int num341 = 0; num341 < 5; num341++)
			{
				...
				{
					mod.DustType<Dusts.AkumaDustLight>(),
                    mod.DustType<Dusts.AkumaADustLight>()
                });
