    ...
    public class Dawnstrike : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			if (projectile.ai[1] == 1f && projectile.ai[0] != 1f)
			{
				...
				{
                    int type = chargeLevel >= 3 ? mod.DustType<Dusts.AkumaADust>() : mod.DustType<Dusts.AkumaDust>();
        ...
        public override void Kill(int timeLeft)
        {
            ...
            if (chargeLevel == 3)
            {
                type = mod.ProjectileType<FireA>();
                damage = projectile.damage * 2;
            }
            else if (chargeLevel == 2)
            {
                type = mod.ProjectileType<AMeteor>();
                damage = projectile.damage * 1.5f;
            }
            else
            {
                type = mod.ProjectileType<MeteorF>();
            }
