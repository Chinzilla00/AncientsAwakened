...
namespace AAMod.Projectiles.Shen
{
    public class SkyfallR : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            if (TileHit)
            {
                Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y - 26, projectile.velocity.X, projectile.velocity.Y, mod.ProjectileType("MeteorStrikeRed"), projectile.damage, projectile.knockBack, projectile.owner, 0f, 0f);
            }
            if (EnemyHit)
            {
                Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y - 26, projectile.velocity.X, projectile.velocity.Y, mod.ProjectileType("MeteorBoomRed"), projectile.damage, projectile.knockBack, projectile.owner, 0f, 0f);
            }
        }
    }
}