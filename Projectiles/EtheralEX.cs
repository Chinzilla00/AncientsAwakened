...
namespace AAMod.Projectiles
{
    public class EtheralEX : ModProjectile
    {
			...
			if (flag10 && Main.myPlayer == projectile.owner)
			{
				...
				if (flag12)
				{
					if (projectile.ai[0] == 1f)
					{
						...
						for (int l = 0; l < 7; l++)
						{
							Projectile.NewProjectile(center3.X, center3.Y, vector12.X, vector12.Y, mod.ProjectileType("EtheralLaserEX"), num29, projectile.knockBack, projectile.owner, l, projectile.whoAmI);
						}
						projectile.netUpdate = true;
					}
				}
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
			...
			Main.spriteBatch.Draw(texture2D14, vector27, new Microsoft.Xna.Framework.Rectangle?(new Microsoft.Xna.Framework.Rectangle(0, y7, texture2D14.Width, num215)), projectile.GetAlpha(color25), projectile.rotation, new Vector2(texture2D14.Width / 2f, num215 / 2f), projectile.scale, spriteEffects, 0f);
			...
			{
				Main.spriteBatch.Draw(texture2D14, vector27 + Vector2.UnitY.RotatedBy(num218 * 6.28318548f / 4f, default(Vector2)) * scaleFactor2, new Microsoft.Xna.Framework.Rectangle?(new Microsoft.Xna.Framework.Rectangle(0, y7, texture2D14.Width, num215)), projectile.GetAlpha(color25).MultiplyRGBA(new Microsoft.Xna.Framework.Color(255, 255, 255, 0)) * 0.03f, projectile.rotation, new Vector2(texture2D14.Width / 2f, num215 / 2f), projectile.scale, spriteEffects, 0f);
			}
			return false;
        }
