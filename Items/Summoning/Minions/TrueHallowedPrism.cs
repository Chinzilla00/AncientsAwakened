    ...
    public class TrueHallowedPrism : Minion2
	{
        ...
        public override void AI()
        {
            ...
            if (projectile.ai[0] == 2f)
            {
                ...
                {
                    int num = Dust.NewDust(projectile.Center, 0, 0, 220 + Main.rand.Next(2), projectile.velocity.X, projectile.velocity.Y, 100, Main.DiscoColor, 1f);
            ...
            if (!flag)
            {
                for (int k = 0; k < 200; k++)
                {
                    ...
                    {
                        Vector2 vector3 = projectile.position + projectile.Size * value;
                        float num13 = Vector2.Distance(vector3, center);
                        if (((Vector2.Distance(center, vector) > num13 && num13 < num10) || !flag) && Collision.CanHitLine(projectile.position, projectile.width, projectile.height, projectile.position, projectile.width, projectile.height))
                        {
                            num10 = num13;
                            vector = vector3;
                            flag = true;
            ...
            if (projectile.ai[0] == 0f)
            {
                ...
                int num29 = mod.ProjectileType<TruePrismshot>();
                if (flag)
                {
