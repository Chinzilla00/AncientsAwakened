...
'I uh...borrowed this from a bird friend of mine.'");
        ...
        public override bool UseItem(Player player)
        {
            ...
            {
                if (player.whoAmI == Main.myPlayer && player.itemTime == 0 && player.controlUseItem) if (Main.netMode != 1) BaseUtility.Chat(Language.GetTextValue("Mods.AAMod.Common.ScepterBossFalse1"), Color.Gold, false);
                return true;
            }
            if (NPC.AnyNPCs(ModContent.NPCType<Anubis>()))
            {
                if (player.whoAmI == Main.myPlayer && player.itemTime == 0 && player.controlUseItem) if (Main.netMode != 1) BaseUtility.Chat(Language.GetTextValue("Mods.AAMod.Common.ScepterBossFalse2"), Color.Gold, false);
                return true;
            }
