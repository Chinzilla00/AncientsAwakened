...
Can only be used at night");
        ...
        public override bool CanUseItem(Player player)
        {
            ...
            {
                if (player.whoAmI == Main.myPlayer && player.itemTime == 0) if (Main.netMode != 1) BaseUtility.Chat(Lang.BossSummonsInfo("InterestingClawTrue"), Color.Indigo, false);
                return false;
            }
            if (Main.dayTime)
            {
                if (player.whoAmI == Main.myPlayer && player.itemTime == 0) if (Main.netMode != 1) BaseUtility.Chat(Lang.BossSummonsInfo("InterestingClawFalse"), Color.Indigo, false);
                return false;
            }
            return true;
        }
