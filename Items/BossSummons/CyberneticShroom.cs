...
Can only be used at night");
        ...
        public override bool CanUseItem(Player player)
        {
            if (Main.dayTime)
            {
                if (player.whoAmI == Main.myPlayer) if (Main.netMode != 1) BaseUtility.Chat(Lang.BossSummonsInfo("CyberneticShroomFalse"), new Color(216, 110, 40), false);
                return false;
            }
            if (NPC.AnyNPCs(mod.NPCType<TechnoTruffle>()))
            {
                if (player.whoAmI == Main.myPlayer) if (Main.netMode != 1) BaseUtility.Chat(Lang.BossSummonsInfo("CyberneticShroomTrue"), new Color(216, 110, 40), false);
                return false;
            }
            return true;
        }
