...
Calls upon the Sisters of Discord");
        ...
        public void SpawnBoss(Player player, string name)
        {
            if (Main.netMode != 1)
            {
                ...
                Main.npc[npcID].Center = player.Center - new Vector2(MathHelper.Lerp(-100f, 100f, (float)Main.rand.NextDouble()), 2000f);
        ...
        public void SpawnBoss2(Player player, string name)
        {
            if (Main.netMode != 1)
            {
                ...
                if (NPC.AnyNPCs(bossType)) { return; } //don't spawn if there's already a boss!
                int npcID = NPC.NewNPC((int)player.Center.X, (int)player.Center.Y, bossType, 0);
                Main.npc[npcID].Center = player.Center - new Vector2(2000f, 0);
                Main.npc[npcID].netUpdate2 = true;
            }
        }
