            ...
            if (BasePlayer.HasItem(player, type, 1))
            {
                for (int m = 0; m < 50; m++)
                {
                    ...
                    if (item != null && item.type == type && item.stack >= 1)
                    {
                        ...
                        {
                            Globals.AAModGlobalNPC.SpawnBoss(player, ModContent.NPCType<Greed>(), true, 0, 0, Language.GetTextValue("Mods.AAMod.Common.Greed"));
                        }
                        else
                        {
                            Globals.AAModGlobalNPC.SpawnBoss(player, ModContent.NPCType<GreedSpawn>(), false, new Vector2(i * 16, (j * 16) - 200), Language.GetTextValue("Mods.AAMod.Common.Greed"));
                        }
                    }
                }
            }
