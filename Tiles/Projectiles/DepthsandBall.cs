...
namespace AAMod.Tiles.Projectiles
{
    class DepthsandBall : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, Terraria.ModLoader.ModContent.DustType<Dusts.BogwoodDust>(), 0f, 0f, 0);
        ...
        public override void Kill(int timeLeft)
        {
            if (projectile.owner == Main.myPlayer && !projectile.noDropItem)
            {
                ...
                if (!Main.tile[tileX, tileY].active())
                {
                    ...
                    if (!onMinecartTrack)
                    {
                        WorldGen.PlaceTile(tileX, tileY, Terraria.ModLoader.ModContent.TileType<Depthsand>(), false, true, -1, 0);
                    }
                    if (!onMinecartTrack && Main.tile[tileX, tileY].active() && Main.tile[tileX, tileY].type == Terraria.ModLoader.ModContent.TileType<Depthsand>())
                    {
                        ...
                        if (Main.netMode != 0)
                        {
                            NetMessage.SendData(17, -1, -1, null, 1, tileX, tileY, Terraria.ModLoader.ModContent.TileType<Depthsand>(), 0, 0, 0);
                        }
                    }
                }
            }
