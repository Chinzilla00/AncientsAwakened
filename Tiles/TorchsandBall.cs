...
namespace AAMod.Tiles
{
    class TorchsandBall : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, mod.DustType<Dusts.RazewoodDust>(), 0f, 0f, 0, default(Color), 1f);
        ...
        public override void Kill(int timeLeft)
        {
            if (projectile.owner == Main.myPlayer && !projectile.noDropItem)
            {
                ...
                if (!Main.tile[tileX, tileY].active())
                {
                    ...
                    if (!onMinecartTrack)
                    {
                        WorldGen.PlaceTile(tileX, tileY, mod.TileType<Torchsand>(), false, true, -1, 0);
                    }
                    if (!onMinecartTrack && Main.tile[tileX, tileY].active() && Main.tile[tileX, tileY].type == mod.TileType<Torchsand>())
                    {
                        ...
                        if (Main.netMode != 0)
                        {
                            NetMessage.SendData(17, -1, -1, null, 1, tileX, tileY, mod.TileType<Torchsand>(), 0, 0, 0);
                        }
                    }
                }
            }
