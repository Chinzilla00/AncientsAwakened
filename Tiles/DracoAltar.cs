            ...
            if (!NPC.AnyNPCs(mod.NPCType("Akuma")) && !NPC.AnyNPCs(mod.NPCType("AkumaA")))
            {
                for (int num66 = 0; num66 < 58; num66++)
                {
                    if (player.selectedItem == mod.ItemType<Items.BossSummons.DraconianSigil>() && player.inventory[player.selectedItem].stack > 0)
                    {
                        if (!Main.dayTime)
                        {
                            if (Main.netMode != 1) BaseUtility.Chat(Lang.TilesInfo("DracoAltar1"), new Color(180, 41, 32), false);
                            return;
                        }
                        if (NPC.AnyNPCs(mod.NPCType<Akuma>()))
                        {
                            if (Main.netMode != 1) BaseUtility.Chat(Lang.TilesInfo("DracoAltar2"), new Color(180, 41, 32), false);
                            return;
                        }
                        if (NPC.AnyNPCs(mod.NPCType<AkumaA>()))
                        {
                            if (Main.netMode != 1) BaseUtility.Chat(Lang.TilesInfo("DracoAltar2"), new Color(0, 191, 255), false);
                            return;
                        }
                        ...
                        {
                            if (Main.netMode != 1) BaseUtility.Chat(Lang.TilesInfo("DracoAltar3"), new Color(180, 41, 32));
                        }
                        if (AAWorld.downedAkuma)
                        {
                            if (Main.netMode != 1) BaseUtility.Chat(Lang.TilesInfo("DracoAltar4"), new Color(180, 41, 32));
                        }
