...
namespace AAMod.Tiles.Furniture.Terra
{
    public class TerraLantern : ModTile
	{
		...
		{
			Item.NewItem(i * 16, j * 16, 48, 32, mod.ItemType("TerraLantern"));
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            for (int k = 0; k < 7; k++)
            {
                ...
                Main.spriteBatch.Draw(mod.GetTexture("Tiles/Furniture/Terra/TerraLantern_Flame"), new Vector2(i * 16 - (int)Main.screenPosition.X + offsetX - (width - 16f) / 2f + x, j * 16 - (int)Main.screenPosition.Y + offsetY + y) + zero, new Rectangle(frameX, frameY, width, height), color, 0f, default, 1f, SpriteEffects.None, 0f);
            }
        }
    }
}
