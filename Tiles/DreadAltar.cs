...
namespace AAMod.Tiles
{
    class DreadAltar : ModTile
    {
        ...
        public override void RightClick(int i, int j)
        {
            ...
            {
                if (player.whoAmI == Main.myPlayer) if (Main.netMode != 1) BaseUtility.Chat(Lang.TilesInfo("DreadAltar2"), new Color(45, 46, 70), false);
                return;
            }
            if (NPC.AnyNPCs(mod.NPCType("YamataA")))
            {
                if (player.whoAmI == Main.myPlayer) if (Main.netMode != 1) BaseUtility.Chat(Lang.TilesInfo("DreadAltar2"), new Color(146, 30, 68), false);
                return;
            }
            ...
            if (!NPC.AnyNPCs(mod.NPCType("Yamata")) && !NPC.AnyNPCs(mod.NPCType("YamataA")))
            {
                for (int num66 = 0; num66 < 58; num66++)
                {
                    if (player.inventory[num66].type == mod.ItemType<Items.BossSummons.DreadRune>() && player.inventory[num66].stack > 0)
                    {
                        ...
                        {
                            if (Main.netMode != 1) BaseUtility.Chat(Lang.TilesInfo("DreadAltar3"), new Color(45, 46, 70));
                        }
                        if (AAWorld.downedYamata)
                        {
                            if (Main.netMode != 1) BaseUtility.Chat(Lang.TilesInfo("DreadAltar4"), new Color(45, 46, 70));
                        }
