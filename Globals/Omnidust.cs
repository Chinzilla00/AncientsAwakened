...
namespace AAMod
{
    public class Omnidust : ModDust
    {
        public override bool Autoload(ref string name, ref string texture)
        {
            texture = AAMod.BLANK_TEX;
            return mod.Properties.Autoload;
        }

		public override void OnSpawn(Dust dust)
		{
			if (dust.customData != null && dust.customData is GDust)
			{
				AADust aadust = (AADust)dust.customData;
				if(aadust.overrideAI == 10) dust.position.Y -= 10;
				if (aadust.fakeType != -1)
				{
					int oldType = dust.type;
					dust.type = aadust.fakeType;
					GDust.VanillaDustInit(dust);
					dust.type = oldType;			
				}
			}
		}
		...
		public override bool Update(Dust dust)
		{
            try
            {
                if (dust.customData != null && dust.customData is GDust)
                {
                    ...
                    if (gdust.overrideAI == -1)
                    {
                        int oldType = dust.type;
                        dust.type = gdust.fakeType;
                        GDust.VanillaDustAI(dust);
                        dust.type = oldType;
                        return false;
                    }else
                    {
                        gdust.customData = gdust.overrideUpdate(dust, gdust.customData);
                        return false;
                    }
                }
            }
            catch (Exception e) { BaseMod.BaseUtility.Chat("DUST UPDATE ERROR: " + e.Message, Color.Red, true); ErrorLogger.Log("DUST UPDATE ERROR: " + e.Message); ErrorLogger.Log(e.StackTrace); ErrorLogger.Log("-------"); }
            return false;
		}

		public static bool normalAlpha = false;
        public override Color? GetAlpha(Dust dust, Color lightColor)
        {
			if(normalAlpha) return null;
            try
            {
                updateType = -1;
                if (dust.customData != null && dust.customData is GDust)
                {
                    GDust gdust = (GDust)dust.customData;
                    Texture2D tex = (gdust.overrideTex == null ? Main.dustTexture : (Texture2D)gdust.overrideTex);
					normalAlpha = true;
                    Color drawColor = dust.GetAlpha(gdust.overrideDrawColor == null ? (dust.color != default(Color) ? dust.GetColor(lightColor) : lightColor) : (Color)gdust.overrideDrawColor);
					if(gdust.overrideAI == 10) //gores
					{
						drawColor = dust.GetAlpha(BaseMod.BaseUtility.Colorize(lightColor, (Color)gdust.overrideDrawColor));
					}
					normalAlpha = false;
                    if (gdust.overrideTex == null)
                    {
						Main.spriteBatch.Draw(tex, dust.position - Main.screenPosition, new Microsoft.Xna.Framework.Rectangle?(dust.frame), drawColor, dust.rotation, new Vector2(4, 4), dust.scale, SpriteEffects.None, 0f);			   
                    }else
                    {
						Vector2 offset = (gdust.overrideAI == 10 ? new Vector2(0f, 10f) : default(Vector2));
                        Main.spriteBatch.Draw(tex, dust.position - Main.screenPosition + new Vector2(tex.Width / 2f, tex.Height / 2f) + offset, new Microsoft.Xna.Framework.Rectangle?(dust.frame), drawColor, dust.rotation, new Vector2(tex.Width / 2f, tex.Height / 2f), dust.scale, SpriteEffects.None, 0f);
                    }
                    if (drawColor == Microsoft.Xna.Framework.Color.Black) dust.active = false;
                }
            }
            catch (Exception e) { BaseMod.BaseUtility.Chat("DUST DRAW ERROR: " + e.Message, Color.Red, true); ErrorLogger.Log("DUST DRAW ERROR: " + e.Message); ErrorLogger.Log(e.StackTrace); ErrorLogger.Log("-------"); }
            return new Color(0, 0, 1, 0);
        }

		public static void DustLight(Dust dust, Color color1, bool sc)
		{
			Color lightColor = color1;
			float scalar = (sc ? dust.scale : (255f - (float)dust.alpha) / 255f);
			lightColor.R = (byte)(lightColor.R * Math.Min(1f, scalar));
			lightColor.G = (byte)(lightColor.G * Math.Min(1f, scalar));
			lightColor.B = (byte)(lightColor.B * Math.Min(1f, scalar));
			BaseMod.BaseDrawing.AddLight(dust.position, lightColor);
		}

		public static object[] DustAI(Dust dust, object[] data)
		{
			byte overrideAI = (byte)data[0];
			short dustID = (short)data[1];
			float[] ai = (data.Length > 3 ? new float[] { (float)data[3], (float)data[4], (float)data[5], (float)data[6] } : new float[0]);
			if (data.Length > 2 && data[2] != null && overrideAI != 11 && overrideAI != 16) { DustLight(dust, (Color)data[2], overrideAI != 7 && overrideAI != 8); }
			float oldScale = dust.scale;
			switch (overrideAI)
			{
				case 0: //jungle dust
				{
				...
				case 1: //lightning
				{
					dust.rotation += (float)(Math.PI / 8f) * (1 + Main.rand.Next(3));
					dust.scale *= 0.85f;
					dust.velocity *= 0.92f;
					dust.position += dust.velocity;
					if (dust.scale <= 0.2f) { dust.active = false; }
				...
				case 2:
				case 5:
				case 6: //acid shot/grovite
				{
					dust.rotation += (float)(Math.PI / 16f) * dust.scale;
					dust.scale *= overrideAI == 6 ? 0.8f : overrideAI == 5 ? 0.87f : 0.96f;
					dust.velocity *= 0.98f;
					dust.position += dust.velocity;
					if (dust.scale <= 0.2f) { dust.active = false; }
					break;
				...
				case 4: //greek swords
				{
					if (overrideAI == 4) { DustLight(dust, Color.Lerp(new Color(94, 150, 70), new Color(242, 191, 96), (dust.scale * 0.5f) / 1f), true); }
					dust.rotation += (float)(Math.PI / 64f);
					dust.scale *= 0.95f;
					dust.velocity *= 0.75f;
					dust.position += dust.velocity;
					if (dust.scale <= 0.2f) { dust.active = false; }
				...
				{
					dust.noGravity = true;
					int alpha = dust.alpha > 200 ? 200 : 255 - dust.alpha;
					((GDust)dust.customData).overrideDrawColor = new Color(255, 255, 255, alpha);
					dust.rotation += (float)(Math.PI / 128f) * (1 + Main.rand.Next(3)) * (dustID % 2 == 0 ? -1f : 1f);
					dust.scale *= 0.99f;
					dust.velocity.X *= 0.95f;
					dust.velocity.Y *= 0.95f;
					dust.position += dust.velocity;
					dust.alpha += 2;
					if (dust.alpha >= 250 || dust.scale <= 0.15f) { dust.active = false; }
					break;
				}
				case 8: //petal dust
				{
					ai[0]++;
					dust.rotation = (dust.velocity.X * 0.25f) + (ai[2] == 1 ? 0.785f : -0.785f);
					dust.velocity.Y = ai[1];
					dust.velocity.X = (float)Math.Sin(ai[0] * 0.08f) * 2.5f;
					Vector2 pos = dust.position + dust.velocity;
					int tileX = (int)(pos.X / 16f), tileY = (int)(pos.Y / 16f);
					tileX = Math.Max(0, Math.Min(Main.maxTilesX, tileX)); tileY = Math.Max(0, Math.Min(Main.maxTilesY, tileY));
					Tile tile = Main.tile[tileX, tileY];
					if (tile != null && !tile.nactive() && tile.liquid > 0)
					{
						dust.active = false;
					}else
					{
				...
				case 9: //rotational explosion
				{
					dust.rotation += (float)(Math.PI / 32f) * dust.scale;
					dust.scale *= 0.94f;
					dust.velocity += BaseMod.BaseUtility.RotateVector(default(Vector2), new Vector2(0.5f, 0f), BaseMod.BaseUtility.RotationTo(default(Vector2), dust.velocity) + (float)(Math.PI / 3f));
					dust.velocity *= 0.9f;
					dust.position += dust.velocity;
					if (dust.scale <= 0.2f) { dust.active = false; }
					break;
				}
                case 10: //special gore
                {
					...
					if(!dust.noGravity)
					{
						...
						dust.velocity = Collision.TileCollision(dust.position, dust.velocity, dust.frame.Width, dust.frame.Height);
					}else
					{
						dust.velocity *= 0.96f;
						dust.scale *= 0.99f;
					...
					dust.position += dust.velocity;		
					ai[0]++;
					if(ai[0] > (dust.noGravity ? 30 : 200)){ ai[0] = 200; dust.alpha += (dust.noGravity ? 3 : 2); }
                    if (dust.alpha >= 255) dust.active = false;
                    break;
                }
				case 11: //rainbow
				{
                    dust.velocity *= Math.Max(0.9f, Math.Min(1f, dust.scale));
					//dust.rotation += 0.2f * dust.scale;
					dust.scale *= 0.95f;
					dust.position += dust.velocity;	
					DustLight(dust, dust.color, true);
					if (dust.scale <= 0.2f) { dust.active = false; }					
					break;
				}
				case 12: //fade-in then shrink
				{
                    dust.velocity *= Math.Max(0.9f, Math.Min(1f, dust.scale));
			...
			if (data.Length > 3){ data[3] = ai[0]; data[4] = ai[1]; data[5] = ai[2]; data[6] = ai[3]; }
			
			/*if(oldScale != dust.scale)
			{
				float scaleDiff = oldScale - dust.scale;
				dust.position -= (scaleDiff * (dust.frame != null ? dust.frame.Size() : new Vector2(4f, 4f))) * 0.25f;
			}*/
            return data;
		}
    }
    ...
    public class GDust
    {
        ...
        public GDust(int fType, Texture2D tex = null, Color? color = null, int ai = -1) 
        {
            fakeType = fType;
            overrideTex = tex;
            overrideColor = color;
