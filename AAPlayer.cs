...
namespace AAMod
{
    public partial class AAPlayer : ModPlayer
    {
        ...
        public override void UpdateBiomes()
        {
            ...
            ZoneMire = (AAWorld.mireTiles > 100) || BaseAI.GetNPC(player.Center, Terraria.ModLoader.ModContent.NPCType<Yamata>(), 5000) != -1 || BaseAI.GetNPC(player.Center, Terraria.ModLoader.ModContent.NPCType<YamataA>(), 5000) != -1;
            ZoneInferno = AAWorld.infernoTiles > 100 || BaseAI.GetNPC(player.Center, Terraria.ModLoader.ModContent.NPCType<Akuma>(), 5000) != -1 || BaseAI.GetNPC(player.Center, Terraria.ModLoader.ModContent.NPCType<AkumaA>(), 5000) != -1;
            ZoneMush = AAWorld.mushTiles > 100;
            Terrarium = AAWorld.terraTiles >= 1;
            ZoneVoid = (AAWorld.voidTiles > 20 && player.ZoneSkyHeight) || (AAWorld.voidTiles > 100 && !player.ZoneSkyHeight) || BaseAI.GetNPC(player.Center, Terraria.ModLoader.ModContent.NPCType<Zero>(), 5000) != -1 || BaseAI.GetNPC(player.Center, Terraria.ModLoader.ModContent.NPCType<ZeroProtocol>(), 5000) != -1;
        ...
        {
            bool useAthena = NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<AthenaA>());
            bool useShenA = NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<ShenA>());
            bool useShen = NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<Shen>()) && !useShenA;
            bool useAkuma = NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<AkumaA>()) || AkumaAltar;
            bool useYamata = NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<YamataA>()) || YamataAltar;
        ...
        public override void OnHitByNPC(NPC npc, int damage, bool crit)
        {
            ...
            if (ChaosMe)
            {
                npc.AddBuff(Terraria.ModLoader.ModContent.BuffType<DragonFire>(), 180);
                npc.AddBuff(Terraria.ModLoader.ModContent.BuffType<HydraToxin>(), 180);
            }
        ...
        public override void PostUpdate()
        {
            ...
            if (NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<AkumaTransition>()))
            {
                int n = BaseAI.GetNPC(player.Center, Terraria.ModLoader.ModContent.NPCType<AkumaTransition>(), -1);
                NPC akuma = Main.npc[n];

                if (akuma.ai[0] >= 660)
                {
                    player.AddBuff(Terraria.ModLoader.ModContent.BuffType<BlazingPain>(), 2);
                }
            }
            else if (NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<AkumaA>()))
            {
                player.AddBuff(Terraria.ModLoader.ModContent.BuffType<BlazingPain>(), 2);
            }

            if (BasePlayer.HasAccessory(player, Terraria.ModLoader.ModContent.ItemType<Items.Vanity.HappySunSticker>(), true, true))
            {
            ...
            if (NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<NPCs.Bosses.Equinox.DaybringerHead>()) || NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<NPCs.Bosses.Equinox.NightcrawlerHead>()))
            {
                TimeScale = 0;
            }
            ...
            if (Orbiters)
            {
                ...
                {
                    player.minionDamage += AAGlobalProjectile.CountProjectiles(Terraria.ModLoader.ModContent.ProjectileType<Projectiles.AH.FireOrbiter>()) * .1f;
                    ...
                    if (Main.netMode != 2 && Main.LocalPlayer.miscCounter % 3 == 0)
                    {
                        for (int m = 0; m < Spheres.Length; m++)
                        {
                            ...
                            {
                                int dustID = Dust.NewDust(projectile.position, projectile.width, projectile.height, Terraria.ModLoader.ModContent.DustType<Dusts.AkumaDustLight>());
            ...
            if (!WorldgenReminder)
            {
                if (Main.rand.Next(8) == 0)
                {
                    if (Main.netMode != 1)
                    {
                        BaseUtility.Chat("Hey uh...kid? Correct me if I'm wrong, but I think your world didn't generate with Ancients Awakened stuff in it. I'd make a new one if I were you.", new Color(180, 41, 32), false);
                    }
                }
                else if (Main.rand.Next(8) == 1)
                {
                    if (Main.netMode != 1)
                    {
                        BaseUtility.Chat("YOU IMBECILE! YOU DIDN'T GENERATE ANCIENTS AWAKENED CONTENT! MAKE A NEW WORLD NOW! REEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE", new Color(45, 46, 70), false);
                    }
                }
                else if (Main.rand.Next(8) == 2)
                {
                    if (Main.netMode != 1)
                    {
                        BaseUtility.Chat("ERR0R. W0RLD D0ES N0T APPEAR T0 C0NTAIN AAM0D.TM0D C0NTENT. PLEASE GENERATE A NEW W0RLD.", new Color(255, 0, 0), false);
                    }
                }
                else if (Main.rand.Next(8) == 3)
                {
                    if (Main.netMode != 1)
                    {
                        BaseUtility.Chat("HEY! You didn't generate Ancients Awakened stuff in this world! Generate a new world before I blast you to mars!", new Color(102, 20, 48), false);
                    }
                }
                else if (Main.rand.Next(8) == 4)
                {
                    if (Main.netMode != 1)
                    {
                        BaseUtility.Chat("Hey, uh...I don't see any Ancients Awakened content in this world. Might be smart to make a new world or whatever...", new Color(72, 78, 117), false);
                    }
                }
                else if (Main.rand.Next(8) == 5)
                {
                    if (Main.netMode != 1)
                    {
                        BaseUtility.Chat("Hey. You. Interdimensional being. You might have forgotten to make a new world after downloading the mod. Make a new world if you want all the mod's content.", new Color(128, 0, 0), false);
                    }
                }
                else if (Main.rand.Next(8) == 6)
                {
                    if (Main.netMode != 1)
                    {
                        BaseUtility.Chat("Make...new world....or mushmad...will squish...little terrarian...", new Color(216, 110, 40), false);
                    }
                }
            ...
            if (NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<Yamata>()))
            {
                player.AddBuff(Terraria.ModLoader.ModContent.BuffType<YamataGravity>(), 10, true);
            }

            if (NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<YamataA>()))
            {
                player.AddBuff(Terraria.ModLoader.ModContent.BuffType<YamataAGravity>(), 10, true);
            }
            ...
            if (player.GetModPlayer<AAPlayer>().ZoneMire || player.GetModPlayer<AAPlayer>().ZoneRisingMoonLake)
            {
                if (Main.dayTime && !AAWorld.downedYamata)
                {
                    ...
                    {
                        player.AddBuff(Terraria.ModLoader.ModContent.BuffType<Clueless>(), 5);
                    }
                }
            }
            ...
            {
                player.AddBuff(Terraria.ModLoader.ModContent.BuffType<Terrarium>(), 2);
            ...
            if (NPC.AnyNPCs(Terraria.ModLoader.ModContent.NPCType<ZeroProtocol>()))
            {
            ...
            if (Assassin)
            {
                ...
                if (flag14)
                {
                    if (Main.rand.Next(2) == 0)
                    {
                        ...
                        Dust dust = Main.dust[Dust.NewDust(player.Center - vector * 30f, 0, 0, Terraria.ModLoader.ModContent.DustType<Dusts.AbyssDust>(), 0f, 0f, 0)];
                    ...
                    if (Main.rand.Next(2) == 0)
                    {
                        ...
                        Dust dust2 = Main.dust[Dust.NewDust(player.Center - vector2 * 30f, 0, 0, Terraria.ModLoader.ModContent.DustType<Dusts.AbyssDust>(), 0f, 0f, 0)];
        ...
        public void AAHorizontalMovement()
        {
            ...
            if (player.controlLeft && player.velocity.X > -player.accRunSpeed && player.dashDelay >= 0)
            {
                if (player.velocity.X < -runSpeed && player.velocity.Y == 0f && !player.mount.Active)
                {
                    if (AADash == 1)
                    {
                        int dust = Dust.NewDust(new Vector2(player.position.X - 4f, player.position.Y), player.width + 8, 4, Terraria.ModLoader.ModContent.DustType<Feather>(), -player.velocity.X * 0.5f, player.velocity.Y * 0.5f, 50, default, 1.5f);
            ...
            if (player.dashDelay < 0)
            {
                ...
                if (AADash == 1)
                {
                    for (int k = 0; k < 2; k++)
                    {
                        ...
                        {
                            num12 = Dust.NewDust(new Vector2(player.position.X, player.position.Y + player.height - 4f), player.width, 8, Terraria.ModLoader.ModContent.DustType<Feather>(), 0f, 0f, 100, default, 1);
                        }
                        else
                        {
                            num12 = Dust.NewDust(new Vector2(player.position.X, player.position.Y + player.height / 2 - 8f), player.width, 16, Terraria.ModLoader.ModContent.DustType<Feather>(), 0f, 0f, 100, default, 1);
                        }
            ...
            else if (AADash > 0 && !player.mount.Active)
            {
                if (AADash == 1)
                {
                    ...
                    if (DashAttempt)
                    {
                        ...
                        {
                            int num18 = Dust.NewDust(new Vector2(player.position.X, player.position.Y), player.width, player.height, Terraria.ModLoader.ModContent.DustType<Feather>(), 0f, 0f, 100, default, 1);
        ...
        public static void AshRain(Player player)
        {
            ...
            if ((player.GetModPlayer<AAPlayer>().ZoneInferno || player.GetModPlayer<AAPlayer>().ZoneRisingSunPagoda) && player.GetModPlayer<AAPlayer>().AshCurse)
            {
                ...
                {
                    player.AddBuff(Terraria.ModLoader.ModContent.BuffType<BurningAsh>(), 5);
                }
                    ...
                    while (num4 < num3)
                    {
                        try
                        {
                            ...
                            if (Main.rand.Next(maxValue) == 0)
                            {
                                ...
                                {
                                    int dust = Dust.NewDust(new Vector2(num5, num6), 10, 10, Terraria.ModLoader.ModContent.DustType<Dusts.AshRain>(), 0f, 0f, 0);
                    ...
                    while (num4 < num3)
                    {
                        try
                        {
                            ...
                            if (Main.rand.Next(maxValue) == 0)
                            {
                                ...
                                if (Main.tile[num7, num8] != null && Main.tile[num7, num8].wall == 0)
                                {
                                    int dust = Dust.NewDust(new Vector2(num5, num6), 10, 10, Terraria.ModLoader.ModContent.DustType<Dusts.Discord>(), 0f, 0f, 0);
                                    Main.dust[dust].velocity.Y = 3f + Main.rand.Next(30) * 0.1f;
        ...
        public override void ProcessTriggers(TriggersSet triggersSet)
        {
            if (AAMod.Rift.JustPressed)
            {
                ...
                for (int m = 0; m < 58; m++)
                {
                    if (player.inventory[m].type == Terraria.ModLoader.ModContent.ItemType<Items.Usable.RiftMirror>())
                    {
                        player.Spawn();
                    }
                }
            ...
            if (SagShield)
            {
                ...
                {
                    player.AddBuff(Terraria.ModLoader.ModContent.BuffType<SagShield>(), 300);
                    SagCooldown = 5400;
                }
            }
        ...
        public override void ModifyHitNPC(Item item, NPC target, ref int damage, ref float knockback, ref bool crit)
        {
            ...
            if (perfectChaosMe)
            {
                target.AddBuff(Terraria.ModLoader.ModContent.BuffType<DiscordInferno>(), 300);
            }
            ...
            if (Naitokurosu)
            {
                int buff = Main.dayTime ? BuffID.Venom : Terraria.ModLoader.ModContent.BuffType<Moonraze>();
                target.AddBuff(buff, 1000);
            }

            if (Duality)
            {
                int buff = Main.dayTime ? BuffID.Daybreak : Terraria.ModLoader.ModContent.BuffType<Moonraze>();
                target.AddBuff(buff, 1000);
            }
            ...
            {
                target.AddBuff(Terraria.ModLoader.ModContent.BuffType<DragonFire>(), 600);
            }
            ...
            {
                target.AddBuff(Terraria.ModLoader.ModContent.BuffType<HydraToxin>(), 600);
            }
            else if (HeartS && player.statLife < (player.statLifeMax / 3))
            {
                target.AddBuff(Terraria.ModLoader.ModContent.BuffType<Moonraze>(), 600);
            }
        ...
        public override void DrawEffects(PlayerDrawInfo drawInfo, ref float r, ref float g, ref float b, ref float a, ref bool fullBright)
        {
            ...
            if (discordInferno)
            {
                for (int i = 0; i < 2; i++)
                {
                    int dust = Dust.NewDust(drawInfo.position - new Vector2(2f, 2f), player.width, player.height, Terraria.ModLoader.ModContent.DustType<Dusts.Discord>(), 0f, -2.5f, 0);
            ...
            if (shroomed)
            {
                for (int i = 0; i < 2; i++)
                {
                    int dust = Dust.NewDust(drawInfo.position - new Vector2(2f, 2f), player.width, player.height, Terraria.ModLoader.ModContent.DustType<Dusts.ShroomDust>(), 0f, -2.5f, 0);
            ...
            if (riftbent)
            {
                ...
                {
                    int dust = Dust.NewDust(drawInfo.position - new Vector2(2f, 2f), player.width, player.height, Terraria.ModLoader.ModContent.DustType<Dusts.CthulhuAuraDust>(), 0f, -2.5f, 0);
        ...
        public override void ModifyHitNPCWithProj(Projectile proj, NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
        {
            ...
            if (proj.magic)
            {
                if (MoonSet)
                {
                    target.AddBuff(Terraria.ModLoader.ModContent.BuffType<Moonraze>(), 300);
                }

                if (zeroSet)
                {
                    target.AddBuff(Terraria.ModLoader.ModContent.BuffType<BrokenArmor>(), 1000);
                }
